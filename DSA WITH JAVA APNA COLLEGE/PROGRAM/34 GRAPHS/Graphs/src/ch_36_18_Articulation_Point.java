...
public class ch_36_18_Articulation_Point {
    ...
    public static void dfs(ArrayList<Edge>graph[] , int curr, int par , int dt[] , int low[]  , int time , boolean vis[] , boolean ap[]){
        ...
        for(int i=0;i<graph[curr].size();i++){
            Edge e = graph[curr].get(i); //(e.src ,e.dec)
            int neigh = e.dec;
            if(neigh==par){
                continue;
            }
            else if(vis[neigh]){ // backedge case : means other vertex se path exist karta hai
...
//            System.out.println("AP : "+curr);
    ...
    public static void getAP(ArrayList<Edge>graph[] , int V){
        ...
        boolean ap[] = new boolean[V]; // for safe from duplicate AP
        for(int i=0;i<V;i++){
            if(!vis[i]){
